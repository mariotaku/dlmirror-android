...
COMPAT_API_EXPORTED struct usb_bus *usb_busses = NULL;
...
COMPAT_API_EXPORTED void usb_init(void)
{
	int r;
	usbi_dbg("");
...
COMPAT_API_EXPORTED void usb_set_debug(int level)
{
	usb_debug = level;
...
COMPAT_API_EXPORTED char *usb_strerror(void)
{
...
COMPAT_API_EXPORTED int usb_find_busses(void)
{
...
COMPAT_API_EXPORTED int usb_find_devices(void)
{
	struct usb_bus *bus;
...
COMPAT_API_EXPORTED struct usb_bus *usb_get_busses(void)
{
	return usb_busses;
}

COMPAT_API_EXPORTED usb_dev_handle *usb_open(struct usb_device *dev)
{
	int r;
	usbi_dbg("");
...
COMPAT_API_EXPORTED int usb_close(usb_dev_handle *dev)
{
	usbi_dbg("");
...
COMPAT_API_EXPORTED struct usb_device *usb_device(usb_dev_handle *dev)
{
	return dev->device;
}

COMPAT_API_EXPORTED int usb_set_configuration(usb_dev_handle *dev, int configuration)
{
	usbi_dbg("configuration %d", configuration);
	return compat_err(libusb_set_configuration(dev->handle, configuration));
}

COMPAT_API_EXPORTED int usb_claim_interface(usb_dev_handle *dev, int interface)
{
	int r;
...
COMPAT_API_EXPORTED int usb_release_interface(usb_dev_handle *dev, int interface)
{
	int r;
	usbi_dbg("interface %d", interface);
...
COMPAT_API_EXPORTED int usb_set_altinterface(usb_dev_handle *dev, int alternate)
{
...
COMPAT_API_EXPORTED int usb_resetep(usb_dev_handle *dev, unsigned int ep)
{
	return compat_err(usb_clear_halt(dev, ep));
}

COMPAT_API_EXPORTED int usb_clear_halt(usb_dev_handle *dev, unsigned int ep)
{
	usbi_dbg("endpoint %x", ep);
	return compat_err(libusb_clear_halt(dev->handle, ep & 0xff));
}

COMPAT_API_EXPORTED int usb_reset(usb_dev_handle *dev)
{
	usbi_dbg("");
...
COMPAT_API_EXPORTED int usb_bulk_read(usb_dev_handle *dev, int ep, char *bytes,
	int size, int timeout)
{
...
COMPAT_API_EXPORTED int usb_bulk_write(usb_dev_handle *dev, int ep, const char *bytes,
	int size, int timeout)
{
	if (ep & USB_ENDPOINT_IN) {
...
COMPAT_API_EXPORTED int usb_interrupt_read(usb_dev_handle *dev, int ep, char *bytes,
	int size, int timeout)
{
	if (!(ep & USB_ENDPOINT_IN)) {
		/* libusb-0.1 will strangely fix up a read request from endpoint
...
COMPAT_API_EXPORTED int usb_interrupt_write(usb_dev_handle *dev, int ep, const char *bytes,
	int size, int timeout)
{
	if (ep & USB_ENDPOINT_IN) {
		/* libusb-0.1 on BSD strangely fix up a write request to endpoint
...
COMPAT_API_EXPORTED int usb_control_msg(usb_dev_handle *dev, int bmRequestType,
...
COMPAT_API_EXPORTED int usb_get_string(usb_dev_handle *dev, int desc_index, int langid,
...
COMPAT_API_EXPORTED int usb_get_string_simple(usb_dev_handle *dev, int desc_index,
	char *buf, size_t buflen)
{
	int r;
...
COMPAT_API_EXPORTED int usb_get_descriptor(usb_dev_handle *dev, unsigned char type,
...
COMPAT_API_EXPORTED int usb_get_descriptor_by_endpoint(usb_dev_handle *dev, int ep,
...
COMPAT_API_EXPORTED int usb_get_driver_np(usb_dev_handle *dev, int interface,
...
COMPAT_API_EXPORTED int usb_detach_kernel_driver_np(usb_dev_handle *dev, int interface)
{
